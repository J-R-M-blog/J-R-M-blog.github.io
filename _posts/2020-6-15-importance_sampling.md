...
This is $E[ln(X^2)]$, where $X \sim \mathcal{N}(0,1)$. This seems challenging enough (I just made this up so hopefully it is sufficiently "too challenging" to serve as a motivating example). To approximate this integral, the Monte Carlo algorithm is as follows:
...
Wait, so what if we just simulate from something easier (like $X^{(i)} \sim q(x)$, maybe?) Then we can just tweak our algorithm:
...
```
    ...
    <matplotlib.legend.Legend at 0x7fcff0fdda90>
...
Now replace $N$ with $\sum_{n=1}^{N}w^{(n)}$. Our new estimator is therefore
...
Notice what happened? The normalization constant disappeared! This is called the _self normalized importance sampler_. So all we need to do in these situations is 

1. Be able to sample from $q(\theta)$.

2. Be able to _evaluate_ $q(\theta), f(\theta), \pi(\theta)$, and $h(\theta)$

The best news, everything still works out! Why? Let's look at the bottom piece of 
...
The above doesn't seem obviously true. It is, though, nonetheless. This is a little off-topic, but here's a simple proof I wanted to do for practice. Those unclear on introductory measure theory can skip. 
